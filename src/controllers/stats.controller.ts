...
export const getAdminStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Calculate trends
  const getTrendData = async (model: Model<Document>, filter: Record<string, unknown> = {}): Promise<{ trend: 'up' | 'down'; change: string }> => {
    const now = new Date();
    ...
    let changePercent = 0;
    if (lastMonthCount > 0) {
      changePercent = ((thisMonthCount - lastMonthCount) / lastMonthCount) * 100;
  ...
  const courseTrend = await getTrendData(Course as unknown as Model<Document>);
  const enrollmentTrend = await getTrendData(Enrollment as unknown as Model<Document>);
  const instructorTrend = await getTrendData(User as unknown as Model<Document>, { role: 'instructor' });
  ...
  const recentCourses = await Course.find().sort({ createdAt: -1 }).limit(5).select('title createdAt');
  ...
  ].sort((a, b) => b.time.getTime() - a.time.getTime()).slice(0, 5).map(item => ({
    id: item.id,
    action: item.action,
  ...
  const categoryDistribution = await Course.aggregate([
    ...
    },
    {
      $project: {
        label: '$_id',
        value: { $multiply: [{ $divide: ['$count', totalCourses || 1] }, 100] },
        _id: 0
      }
    }
  ]);
  ...
  res.status(200).json({
    ...
    data: {
      ...
      enrollmentsByMonth: enrollmentsByMonth.map(item => ({
        label: new Date(item._id.year, item._id.month - 1).toLocaleString('default', { month: 'short' }),
        value: item.count
      })),
...
export const getStudentStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  const completedCourses = await Enrollment.countDocuments({ student: studentId, progress: 100 });
  const inProgressCourses = await Enrollment.countDocuments({ student: studentId, progress: { $lt: 100 } });
